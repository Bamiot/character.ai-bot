...
module.exports = {
  ...
  async execute(client, interaction, characterAI) {
    try {
      ...
      const character = await characterAI.fetchCharacter(
        client.activeCharacter
      ); // Get character by charID

      await character.createDM(false); // Creates a new dm, without the AI replying

      return interaction.reply(
        "Messages have been saved and a new DM has been opened."
      );
    } catch (error) {
      // If something goes wrong:
      console.log(error);
      return interaction.reply(
        "Something went wrong, chat has not been cleared."
